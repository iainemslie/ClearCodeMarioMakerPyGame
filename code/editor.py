...
class Editor:
    def __init__(self, land_tiles, switch):
        ...
        # music
        self.editor_music = pygame.mixer.Sound(join('audio', 'Explorer.ogg'))
        self.editor_music.set_volume(0.4)
        self.editor_music.play(loops=-1)

    # support
    def get_current_cell(self, obj=None):
        distance_to_origin = vector(mouse_pos()) \
            - self.origin if not obj else vector(obj.distance_to_origin) - self.origin
            ...
            if cell in self.canvas_data:
                ...
                for name, side in NEIGHBOR_DIRECTIONS.items():
                    ...
                    if neighbour_cell in self.canvas_data:
                        if self.canvas_data[cell].has_water \
                                and name == 'A' \
                                and self.canvas_data[neighbour_cell].has_water:
                            self.canvas_data[cell].water_on_top = True

                    # terrain neighbours
                        if self.canvas_data[neighbour_cell].has_terrain:
                            self.canvas_data[cell].terrain_neighbours.append(
                                name)

    def imports(self):
        self.water_bottom = load(
            join('graphics', 'terrain', 'water', 'water_bottom.png')).convert_alpha()
        self.sky_handle_surf = load(
            join('graphics', 'cursors', 'handle.png')).convert_alpha()

        # animations
        self.animations = {3: {'frame index': 0,
                               'frames': ['surfaces'],
                               'length': 3}}
        ...
        # preview
        self.preview_surfs = {key: load(
            value['preview']) for key, value in EDITOR_DATA.items() if value['preview']}
            ...
            if selected_object:
                ...
                pygame.draw.lines(self.display_surface,
                                  ...
                                  ((rect.left, rect.top + size),
                                   rect.topleft, (rect.left + size, rect.top)),
                                  width)
                # topright
                pygame.draw.lines(self.display_surface,
                                  color,
                                  ...
                                  ((rect.right - size, rect.top),
                                   rect.topright, (rect.right, rect.top + size)),
                                  width)
                # bottomright
                pygame.draw.lines(self.display_surface,
                                  color,
                                  ...
                                  ((rect.right, rect.bottom - size),
                                   rect.bottomright, (rect.right - size, rect.bottom)),
                                  width)
                # bottomeleft
                pygame.draw.lines(self.display_surface,
                                  color,
                                  ...
                                  ((rect.left, rect.bottom - size),
                                   rect.bottomleft, (rect.left + size, rect.bottom)),
                                  width)
            else:

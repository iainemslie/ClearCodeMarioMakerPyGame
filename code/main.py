...
class Main:
    ...
    def imports(self):
        ...
        self.water_top_animation = import_folder(
            join('graphics', 'terrain', 'water', 'animation'))

        # coins
        self.gold = import_folder(join('graphics', 'items', 'gold'))
        self.silver = import_folder(join('graphics', 'items', 'silver'))
        ...
        self.player_graphics = {folder: import_folder(join('graphics', 'player', folder))
                                for folder in list(walk(join('graphics', 'player')))[0][1]}

        # clouds
        self.clouds = import_folder(join('graphics', 'clouds'))
        ...
        self.level_sounds = {
            ...
            'music': pygame.mixer.Sound(join('audio', 'SuperHero.ogg')),
        }
